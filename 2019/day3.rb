class Wire
  ...
  class << self
    ...
    def closest_distance(a, b)
      intersection(a, b).map { |(a, b)| a.abs + b.abs }.min
    end

    def closest_with_delay(a, b)
      steps = intersection(a, b).map do |cord|
        steps_a = a.trace.index(cord) + 1
        steps_b = b.trace.index(cord) + 1
        steps_a + steps_b
      end
      steps.min
    end
  ...
  def process_cords(cords)
    cords.each do |cord|
      direction = cord[0]
      steps = cord[1..-1].to_i
      add_steps(direction, steps)
    end
  end

  private

  def add_steps(direction, steps)
    end_x = x
    end_y = y
    ratio = 0
    ...
    trace_path(end_x, end_y, ratio)
  end

  def trace_path(x2, y2, ratio)
    dx = (x2 - x).abs
    dy = (y2 - y).abs
